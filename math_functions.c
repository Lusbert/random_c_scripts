...
int main() {
    ...
    int C = round(
        3.14); // -> 3         rounds the number duh, basically 2.3 rounds to and 2.5 rounds to 3 when using round()
    int D = ceil(3.14);    // -> 4         rounds it up
    int E = floor(3.99);   // -> 3         always round it down
    double F = fabs(-100); // -> 100       find absoloute value of a number (how far away it is from 0 (so it will take
